...
const getFamilyContextByRecipient = (recipient: string): FamilyContext | null => {
  ...
  const family = db
    ...
    .get(alias) as { id: Buffer; routing_alias: string } | undefined;
...
const parseJsonFromText = (text: string): { events: ParsedEventCandidate[] } => {
  const parsed = JSON.parse(text) as { events?: ParsedEventCandidate[] };
  return {
    events: Array.isArray(parsed.events) ? parsed.events : []
  };
};

const extractJsonPayload = async (message: string, emailBody: string): Promise<{ events: ParsedEventCandidate[] }> => {
...
export const parseEmailAndPersistEvents = async (payload: {
  ...
  const insert = db.prepare(
    `INSERT INTO parsed_events (id, family_id, child_id, activity_id, title, description, start_time, end_time, location, source_email_id, needs_review)
