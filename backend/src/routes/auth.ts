...
const findUserByEmail = (email: string) =>
  db.prepare('SELECT id, email, role, family_id FROM users WHERE lower(email) = lower(?) LIMIT 1').get(email) as
    | { id: Buffer; email: string; role: 'admin' | 'member'; family_id: Buffer }
    | undefined;
