...
settingsRouter.get('/profile', requireUserSession, (req, res) => {
  ...
  const family = db
    ...
    .get(familyId) as { routing_alias: string; created_at: string } | undefined;

  const children = db
    .prepare('SELECT id, first_name, class_name, teacher_name FROM children WHERE family_id = ?')
    .all(familyId)
    .map((row: any) => ({ ...row, id: uuidBufferToString(row.id) }));

  const activities = db
    .prepare(
      `SELECT activities.id, activities.child_id, activities.name, activities.activity_type
       FROM activities INNER JOIN children ON children.id = activities.child_id
       WHERE children.family_id = ?`
    )
    .all(familyId)
    .map((row: any) => ({ ...row, id: uuidBufferToString(row.id), child_id: uuidBufferToString(row.child_id) }));
...
settingsRouter.post('/children', requireUserSession, (req, res) => {
  const familyId = uuidStringToBuffer(req.user!.familyId);
  const { firstName, className, teacherName } = req.body as {
    firstName?: string;
    className?: string;
    teacherName?: string;
  };
  ...
  db.prepare('INSERT INTO children (id, family_id, first_name, class_name, teacher_name) VALUES (?, ?, ?, ?, ?)').run(
    childId,
    familyId,
    firstName,
    className ?? null,
...
settingsRouter.post('/activities', requireUserSession, (req, res) => {
  const { childId, name, activityType } = req.body as {
    childId?: string;
    name?: string;
    activityType?: 'school_club' | 'hobby' | 'sports' | 'volunteering';
  };
  ...
  db.prepare('INSERT INTO activities (id, child_id, name, activity_type) VALUES (?, ?, ?, ?)').run(
    activityId,
    uuidStringToBuffer(childId),
    name,
    activityType
  );
...
settingsRouter.get('/routing-alias', requireUserSession, (req, res) => {
  const family = db
    ...
    .get(uuidStringToBuffer(req.user!.familyId)) as { routing_alias: string } | undefined;
...
settingsRouter.post('/invites', requireUserSession, async (req, res) => {
  const familyId = uuidStringToBuffer(req.user!.familyId);
  const { shareName, sharePassword, expiresInHours = 72 } = req.body as {
    shareName?: string;
  ...
  db.prepare(
    `INSERT INTO family_invites (id, family_id, share_name, share_password_hash, link_token_hash, link_expires_at)
     VALUES (?, ?, ?, ?, ?, ?)
  ...
  ).run(generateUuidBuffer(), familyId, shareName, passwordHash, linkTokenHash, expiresAt);
...
settingsRouter.put('/account/password', requireUserSession, async (req, res) => {
  ...
  const user = db
    ...
    .get(uuidStringToBuffer(req.user!.userId)) as { id: Buffer; password_hash: string | null } | undefined;
...
adminRouter.put('/account/email', (req, res) => {
  ...
  ).run('admin_email', email.trim().toLowerCase());
