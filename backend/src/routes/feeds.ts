...
const toIcsDateTuple = (date: Date): [number, number, number, number, number] => [
  date.getUTCFullYear(),
  date.getUTCMonth() + 1,
  date.getUTCDate(),
  date.getUTCHours(),
...
feedsRouter.get('/:routing_alias/calendar.ics', (req, res) => {
  const family = db
    ...
    .get(req.params.routing_alias) as { id: Buffer } | undefined;
...
feedsRouter.get('/:routing_alias/rss', (req, res) => {
  ...
  const emails = db
    .prepare(
      `SELECT id, sender, subject, body_text, processed_at
